...
class PackageInstallerManager(private val context: Context) {
    ...
    suspend fun install(packageName: String, apkFiles: List<File>): Result<Unit> {
        if (apkFiles.isEmpty()) {
            return Result.failure(IllegalArgumentException("No APK files to install."))
        }
            ...
            override fun onReceive(receiverContext: Context, intent: Intent) {
                when (intent.getIntExtra(PackageInstaller.EXTRA_STATUS, PackageInstaller.STATUS_FAILURE)) {
                    ...
                    PackageInstaller.STATUS_PENDING_USER_ACTION -> {
                        intent.getParcelableExtra(Intent.EXTRA_INTENT, Intent::class.java)?.let { confirmation ->
